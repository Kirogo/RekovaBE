...
const UserSchema = new mongoose.Schema(
  {
    ...
    teamMembers: [
      {
        // For supervisors to track their team
        type: mongoose.Schema.Types.ObjectId,
        ref: "User",
      },
    ],
    ...
    capacity: {
      ...
      assignmentPriority: {
        ...
        default: 1,
        min: 1,
        max: 5, // 1 = highest priority, 5 = lowest
      },
    },

    // For team assignment (supervisors only)
    supervisedLoanTypes: [
      {
        type: String,
        enum: [
...
UserSchema.virtual("permissionsSummary").get(function () {
  ...
  if (this.permissions.canApproveTransactions)
    summaries.push("Approve Transactions");
  if (this.permissions.canViewAllPerformance)
    summaries.push("View All Performance");
    ...
    if (this.performanceMetrics) {
      ...
      if (total > 0) {
        ...
        const calculatedRating =
          (successRate * 0.5 + targetProgress * 0.3 + averageScore * 0.2) * 10;
